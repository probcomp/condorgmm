...
def sgld_step_kernel_vec3(
    ...
    state = wp.rand_init(wp.int32(t))
    noise = wp.vec3(wp.randn(state), wp.randn(state), wp.randn(state)) * wp.sqrt(
        2.0 * lr
    )

    params[i] = params[i] - (lr * wp.cw_div(mhat, (sqrt_vhat + eps_vec3)) + noise)


@wp.kernel
def sgld_step_kernel_float(
    ...
    state = wp.rand_init(wp.int32(t))
    noise = wp.randn(state) * wp.sqrt(2.0 * lr)
    params[i] = params[i] - (lr * mhat / (wp.sqrt(vhat) + eps) + noise)
...
class SGLD:
    ...
    def step_detail(g, m, v, lr, beta1, beta2, t, eps, params):
        ...
        if params.dtype == wp.types.float32:
            ...
            wp.launch(
                kernel=sgld_step_kernel_vec3,
                dim=len(params),
                inputs=kernel_inputs,
                device=params.device,
            )
        else:
            raise RuntimeError("Params data type not supported in SGLD step kernels.")
